import { Component, ViewEncapsulation, Injector, ChangeDetectorRef } from "@angular/core";
...
import { FeedTranslationLoaderService } from "../../theme-core/services";
 ...
 */
@Component({
  selector: "feed-base-component",
  encapsulation: ViewEncapsulation.None,
  template: ""
})
export class FeedBaseComponent {

  public FAILED_STATUS: boolean = true;
  public SUCCESS_STATUS: boolean = false;
  ...
  public translationLoader: FeedTranslationLoaderService;
  ...
  constructor(injector: Injector) {
    ...
    this.translationLoader = injector.get(FeedTranslationLoaderService);
  ...
  set isFormSubmitted(value: boolean) {
    // updating the shared variable that is shared across the app, even by those which do not inherit from Feed Base Component
  ...
  protected isSuccessful(message?: string, redirectUrl?: string): void {
    ...
    if (redirectUrl) {
      this.goTo(redirectUrl, this.constantList.DEFAULT_REDIRECTION_WAIT_TIME);
    }
  }
  ...
  canAccessModule(moduleName: string) {
    if (!this.permissionService.canAccessModule(moduleName)) {
      this.redirectToLandingPageByRole(this.userService.user.role.name);
    }
  }
